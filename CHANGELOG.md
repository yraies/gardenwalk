...
- Password verification now uses timing-safe server-side comparisons, reducing timing-attack exposure on both salted and legacy verification paths.
...
- New client-encrypted artifacts now derive keys with 600,000 PBKDF2 iterations while preserving decryption support for older 10,000-iteration payloads.
- API routes now reject malformed form/template/share IDs early, sanitize server-side error logs, and Docker builds exclude `.env*` files plus `data/` from the build context. A new `.env.example` documents deploy-time secrets.
