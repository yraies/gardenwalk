...
- **Compatibility expectation**: The app is now deployed; persisted database artifacts, shared/admin URLs, local browser data, and exported JSON must keep working across updates. Schema/type changes should be additive or include migration/normalization. (F-031)
...
- **Onboarding and accessibility clarity**: The app should better explain its purpose and help new users understand what to do first; templates should include concise descriptions to put users in the right mindset; default template descriptions should stay colocated with their template definitions; response-option meanings must be visible on mobile/touch devices without relying on hover. (F-004, F-030, F-031, F-033)
...
- **Print behavior**: Printed templates and forms should include visible handwritten answer boxes, preserve meaningful color in browser/PDF print output where possible, and show a compact centered one-line answer legend for the normal answer options while omitting the unset option. (F-026)
...
- **Data portability and comparison**: Exported JSON files should be re-importable as new local drafts. A dedicated comparison view should let users compare 2+ published forms from the same parent template side by side, including password-protected shared responses after successful password entry. Comparison columns should show respondent names (not form titles) plus source template names where known. The compare page should not evaluate agreement/disagreement — just show raw answers. (F-018, superseded partially by F-023; F-029, F-032)
- **Form identity, privacy, and comparison semantics**: Forms should persist the originating template name (frozen) and record the respondent's name instead of having an editable form title. Comparison should treat a share link as another access path to the same published form rather than as a second compareable artifact: if a locally published form and its shared link both exist on the same browser, compare selections and suggestions should deduplicate them. Empty compare suggestions should show template names. The product goal is still to help users find areas of agreement, disagreement, or alignment, but as a communication tool rather than via technological evaluation; comparison should show raw answers and let users interpret them. Published/admin form URLs and shared/read-only URLs are distinct concepts. Stored artifact data should be encrypted in the database even when users do not set a password; password protection remains optional. Share flows must not expose admin/delete form ids just to support comparison; use a separate opaque compare identity instead. Published forms should have a shared read-only view with regeneratable URLs. Auto-delete should delete the underlying published form artifact, not just shared access. Any readable form or template should support creating a new local template draft from its structure, and any readable form or template should also support creating a new local fillable form from its structure. (F-023)
...
- **Product name and theming direction**: The app is now "Garden Walk" (evoking a walk through a mental garden of thoughts, expectations, wants, and needs). Four seasonal color themes (Spring, Summer, Autumn, Winter) are now implemented as a global theme switcher with localStorage persistence. Future direction includes per-template customizable block/background colors, semantic side-rail button colors derived from the active theme, and full semantic answer chip coloring via theme tiers. Spring's destructive color uses a warm tulip red. Winter is pastel and light. Autumn should read as a more distinct harvest/woodland theme rather than feeling too close to summer. (F-021, F-024, F-027)
...
- **Deployment preference**: Provide a proper Quadlet container file for Podman deployments that binds the app to local HTTP only so it can sit behind a Caddy reverse proxy, and prefer a named Podman volume over a host bind mount for persistent data. (F-028)
