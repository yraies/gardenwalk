...
- **REQ-9: Unified Access Control** — Artifact access should use one coherent model built around possession of the correct access URL plus optional password protection, rather than separate share-password and form-password systems. Status: ✅ implemented.
...
- **REQ-32: Optional Secondary Answer Row** — Templates may optionally define a second template-wide answer schema independent of the primary schema. When defined, a finalized template carries this secondary schema into every form created from it. Fillers opt into the secondary row per form via a form-level toggle; the toggle is locked once the form is published. In read-only views (shared, admin, compare) the secondary row is shown only when the form has at least one non-unset secondary answer. CSV export includes a Secondary column when the form has any secondary answer. Older artifacts without a secondary schema must continue loading. Status: ✅ implemented.
